...
def build_helper_dex(build: Path, env: dict[str, str], extra_class_dirs: tuple[Path, ...] = ()) -> Path:
    ...
    # d8.bat expands every positional argument through cmd.exe.  The helper
    # now contains enough classes that the expanded command line exceeds the
    # Windows limit, so pass the same arguments through D8's response-file
    # syntax instead.
    d8_args = build / "d8.args"
    d8_args.write_text(
        "\n".join([
            "--min-api",
            "24",

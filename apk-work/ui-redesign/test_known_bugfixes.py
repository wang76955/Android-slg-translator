...
'''
    ...
    def test_local_mlkit_route_bypasses_api_key_and_uses_native_bridge(self):
        import patch_local_ui
...
''' + patch_local_ui._LOCAL_HELPERS + r'''
installLocalHooks({providerId:"local",model:"mlkit"});
if(window.__slgLocalEngine!=="mlkit")throw new Error("ML Kit engine not selected");
if(!window.__slgLocalSelected)throw new Error("local provider must be selected");
if(typeof window.__slgLocalTranslate!=="function")throw new Error("local bridge hook missing");
'''
        result = subprocess.run(
            ["node", "-e", helper_contract],
            capture_output=True,
            text=True,
            encoding="utf-8",
            check=False,
        ...
        js, _ = self.patch_assets()
        for token in (
            "localStatus", "localDownload", "translateLocal",
            'localDownload({sourceLang:"en",targetLang:"zh"})',
            'if(window.__slgLocalSelected){try{target.disabled=false',
            'providerId==="local"',
            'Array.from(globalThis.document?.querySelectorAll?.("select")||[]).some(_s=>_s.value==="local")',
        ):
            self.assertIn(token, js)
        self.assertGreaterEqual(
            js.count('Array.from(globalThis.document?.querySelectorAll?.("select")||[]).some(_s=>_s.value==="local")'),
            2,
            "the core React settings select must bypass the API-key gate for local ML Kit",
        )
        self.assertIn(
            'if((globalThis.__slgLocalTranslate||globalThis.__slgLocalTranslateImpl||globalThis.__slgLocalSelected||Array.from(globalThis.document?.querySelectorAll?.("select")||[]).some(_s=>_s.value==="local")||',
            js,
            "the core React provider select must arm the local Lo branch even before the overlay hook runs",
        )
        self.assertNotIn(
            'if(isStart&&target?.disabled){const snap=readTaskSnapshot();',
            js,
            "local provider must not retain the API-key-only start gate",
        )

    def test_local_core_route_falls_back_to_native_bridge_without_overlay_hooks(self):
        """The main task-page selector must work before the settings overlay runs."""
        import patch_local_ui

        js, _ = self.patch_assets()
        lo_start = js.index("async function Lo")
        lo_end = js.index("function Ro", lo_start)
        lo = js[lo_start:lo_end]
        contract = patch_local_ui._CORE_LOCAL_BRIDGE + r'''
globalThis.window=globalThis;
const calls=[];
globalThis.Capacitor={Plugins:{FileManager:{translateLocal:async({texts,engine})=>{
  calls.push({count:texts.length,engine});
  return {translations:Object.fromEntries(texts.map((item,index)=>[item.keyPath,"T"+index]))};
}}}};
globalThis.localStorage={getItem(key){
  return key==="slg-workshop-settings-v1"?JSON.stringify({providerId:"local",model:"mlkit"}):null;
},setItem(){}};
''' + lo + r'''
(async()=>{
  const result=await Lo({
    texts:[{keyPath:"a",text:"A",duplicateKeys:[]},{keyPath:"b",text:"B",duplicateKeys:[]}],
    sourceLang:"en",targetLang:"zh",baseURL:"",apiKey:"",model:"gpt-4o-mini",batchSize:20,
    onProgress:()=>{}
  });
  if(result.successCount!==2)throw new Error("core local fallback did not translate");
  if(calls.length!==1||calls[0].engine!=="mlkit")throw new Error("native ML Kit bridge was not called");
})().catch(error=>{console.error(error);process.exitCode=1});
'''
        result = subprocess.run(
            ["node", "-e", contract],

...
"""
    ...
    def test_file_controller_uses_six_workers_and_serial_local_mode(self):
        self.assertIn(
            "return N},(window.__slgLocalSelected?1:6));if(!N)await wo(!0);if(!N&&(a.length>0||oe)){",
            self.js,
        )
        self.assertNotIn(
    ...
    def test_local_file_controller_serializes_files_for_memory(self):
        self.assertIn(
            "return N},(window.__slgLocalSelected?1:6));if(!N)await wo(!0);if(!N&&(a.length>0||oe)){",
            self.js,
        )
...
"""
    ...
    def test_local_bridge_uses_one_current_chunk(self):
        self.assertNotIn(
            "const chunks=[];for(let i=0;i<items.length;i+=20)chunks.push",
            self.js,
        )
        self.assertIn("const chunk=items.slice(offset,offset+20)", self.js)

    def test_local_translation_has_release_hook(self):
    ...
    def test_translation_diagnostics_have_explicit_trim_hook(self):
        ...
        self.assertIn("__slgTrimTranslationDiagnostics?.({reset:true})", self.js)
        self.assertIn("__slgTrimTranslationDiagnostics?.();", self.js)

    def test_translation_diagnostics_use_compact_record_accumulator(self):
        self.assertIn("__slgAppendCoverageRecords", self.js)
        self.assertNotIn(
            "__slgCoverageRecords=(globalThis.__slgCoverageRecords||[]).concat",
            self.js,
        )
        self.assertIn("__slgAppendCompatibilityRecords", self.js)

    def test_translation_diagnostics_trim_preserves_coverage_summary(self):
        start = self.js.index("globalThis.__slgTrimTranslationDiagnostics=")
        end = self.js.index("\n", start)
        hook = self.js[start:end]
        script = f"""
const report = {{uniqueSourceCount: 3, topMissingFiles: [{{filePath: "game.rpy"}}]}};
globalThis.__slgCoverageRecords = [{{text: "large record"}}];
globalThis.__slgRenpyCompatibilityRecords = [{{text: "compat record"}}];
globalThis.__slgTranslationCollisionEntries = [{{old: "a"}}];
globalThis.__slgTranslationCollisionCandidates = new Map([["a", ["b"]]]);
globalThis.__slgValidatorApprovedTranslations = new Map([["a", "b"]]);
globalThis.__slgRejectedTranslations = new Set(["a"]);
globalThis.__slgCoverageUncertain = ["a"];
globalThis.__slgCoverageClassifications = {{a: "reason"}};
globalThis.__slgTranslationCoverageReport = report;
globalThis.__slgTranslationCoverageReportJson = JSON.stringify(report);
globalThis.__slgBuildCoverage = report;
new Function({json.dumps(hook)})();
globalThis.__slgTrimTranslationDiagnostics();
globalThis.__slgTrimTranslationDiagnostics();
const preserved = globalThis.__slgTranslationCoverageReport === report
  && globalThis.__slgBuildCoverage === report
  && globalThis.__slgTranslationCoverageReportJson === JSON.stringify(report);
const trimmed = globalThis.__slgCoverageRecords.length === 0
  && globalThis.__slgRenpyCompatibilityRecords.length === 0
  && globalThis.__slgTranslationCollisionEntries.length === 0
  && globalThis.__slgTranslationCollisionCandidates.size === 0
  && globalThis.__slgValidatorApprovedTranslations.size === 0
  && globalThis.__slgRejectedTranslations.size === 0
  && Object.keys(globalThis.__slgCoverageClassifications).length === 0;
globalThis.__slgTrimTranslationDiagnostics({{reset: true}});
process.stdout.write(JSON.stringify({{preserved, trimmed, resetReport: globalThis.__slgTranslationCoverageReport === null}}));
"""
        out = json.loads(run_node(script))
        self.assertEqual(out, {"preserved": True, "trimmed": True, "resetReport": True})

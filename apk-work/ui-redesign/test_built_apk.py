...
class BuiltApkTest(unittest.TestCase):
    ...
    def test_manifest_queries_are_minimal_and_same_intent(self):
        ...
        split_intents = re.sub(
            r"(?m)^(\s{4,})E: category \(line=\d+\)",
            lambda match: (
                f"{match.group(1)[:-4]}E: intent (line=999)\n"
                f"{match.group(1)}E: category (line=999)"
            ),
            dump,
            count=1,
        )
        ...
        package_query = re.sub(
            r"(?m)^(\s*)E: intent \(line=\d+\)",
            lambda match: (
                f"{match.group(1)}E: package (line=999)\n"
                f"{match.group(1)}  A: http://schemas.android.com/apk/res/android:name(0x01010003)=\"example.bad\"\n"
                f"{match.group(0)}"
            ),
            dump,
            count=1,
    ...
    def test_mlkit_resources_and_native_translation_library_are_packaged(self):
        self.require_artifact_inputs()
        required = {
        ...
        with zipfile.ZipFile(APK) as archive:
            ...
            for name in required:
                self.assertGreater(archive.getinfo(name).file_size, 0, name)

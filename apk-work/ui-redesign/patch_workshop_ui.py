...
WORKSHOP_RUNTIME = r"""
;(()=>{const ID="workshop-runtime";let shell=null,runtimeRoot=null,sourceButton=null,startButton=null,observer=null,debounceTimer=0,lastSnapshot="",manualIdle=false,retrying=false;
...
function refresh(){if(!shell||manualIdle||retrying)return;const snap=readTaskSnapshot(),key=snapshotKey(snap);if(key===lastSnapshot)return;lastSnapshot=key;setWorkshopState(snap.state,snap)}

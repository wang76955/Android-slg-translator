...
# Keep the visible hero control accessible and keep the auxiliary nav inside the
# runtime shell so task-state selectors can hide it without relying on :has().
WORKSHOP_RUNTIME = WORKSHOP_RUNTIME.replace(
    'start.setAttribute("aria-hidden","true");', ""
).replace("document.body.append(nav)", "app.append(nav)")

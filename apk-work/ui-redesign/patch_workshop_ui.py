...
async function chooseInstalledApp(app){try{const selection=await window.Capacitor.Plugins.FileManager.selectInstalledApp({packageName:app.packageName});closeInstalledApps();await window.__slgLoadSelectedApk(selection)}catch(error){installedError=error?.message||String(error);armModalHistory();if(installedDialog)installedDialog.hidden=false;renderInstalledApps();focusInstalledTarget()}}
function closeInstalledApps(preserveHistory=false){if(installedDialog)installedDialog.hidden=true;if(!preserveHistory)releaseModalHistory();previousSourceFocus?.focus()}
...
function handleWorkshopPopState(){if(modalHistoryClosing){modalHistoryClosing=false;if(modalHistoryRearm){modalHistoryRearm=false;armModalHistory()}return}if(installedDialog&&!installedDialog.hidden){modalHistoryArmed=false;closeInstalledApps(true);return}if(sourceDialog&&!sourceDialog.hidden){modalHistoryArmed=false;closeSourceChooser(true);return}if(settingsOpen){closeSettings();return}if(shell?.dataset.workshopTask==="active"){manualIdle=true;setWorkshopState("idle",{fromBack:true})}}

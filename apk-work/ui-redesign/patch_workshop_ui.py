...
def patch_saves_runtime(js: str) -> str:
    ...
    saves_new = saves_new.replace(
        'let savesSelectedPkg="",savesSelectedLabel="";',
        'let saveExportGamePackage="",saveExportGameLabel="",saveImportGamePackage="",saveImportGameLabel="",saveImportArchiveUri="";',
        1,
    )
    saves_new = saves_new.replace('savesSelectedPkg', 'saveExportGamePackage')
    saves_new = saves_new.replace('savesSelectedLabel', 'saveExportGameLabel')
    saves_new = saves_new.replace('let importSelectedPkg="",importSelectedLabel="";', '')
    saves_new = saves_new.replace('importSelectedPkg', 'saveImportGamePackage')
    saves_new = saves_new.replace('importSelectedLabel', 'saveImportGameLabel')
    saves_new = saves_new.replace(
        'const updateSavesState=()=>{const pkg=saveExportGamePackage;const ready=!!pkg&&!!plugin;exportBtn.disabled=!ready;',
        'function saveTransferBusy(){const state=typeof readTaskSnapshot==="function"?readTaskSnapshot()?.state:"idle";return state==="scanning"||state==="translating"||state==="patching"}const saveTransferBusyMessage=()=>"正在处理任务，请等待完成后再进行存档转移。";const updateSavesState=()=>{const busy=saveTransferBusy();const pkg=saveExportGamePackage;const ready=!!pkg&&!!plugin&&!busy;exportBtn.disabled=!ready;gameBtn.disabled=busy;',
        1,
    )
    saves_new = saves_new.replace(
        'gameBtn.onclick=async()=>{gameBtn.disabled=true;',
        'gameBtn.onclick=async()=>{if(saveTransferBusy()){gameLabel.textContent=saveTransferBusyMessage();return}gameBtn.disabled=true;',
        1,
    )
    saves_new = saves_new.replace(
        'async function refreshSaves(){if(!plugin){',
        'async function refreshSaves(){if(saveTransferBusy()){savesStatus.textContent=saveTransferBusyMessage();return}if(!plugin){',
        1,
    )
    for _action in ('restore', 'share', 'del'):
        saves_new = saves_new.replace(
            f'{_action}.onclick=async()=>{{if(!confirm',
            f'{_action}.onclick=async()=>{{if(saveTransferBusy()){{savesStatus.textContent=saveTransferBusyMessage();return}}if(!confirm',
            1,
        )
    saves_new = saves_new.replace(
    ...
    saves_new = saves_new.replace(
        'importGameBtn.onclick=async()=>{importGameBtn.disabled=true;',
        'importGameBtn.onclick=async()=>{if(saveTransferBusy()){importGameLabel.textContent=saveTransferBusyMessage();return}importGameBtn.disabled=true;',
        1,
    )
    saves_new = saves_new.replace(
        'importOne.onclick=async()=>{importOne.disabled=true;',
        'importOne.onclick=async()=>{if(saveTransferBusy()){importStatus.textContent=saveTransferBusyMessage();return}saveImportArchiveUri=item.path||"";importOne.disabled=true;',
        1,
    )
    saves_new = saves_new.replace(
        'const result=await plugin.importSaveBackup({path:item.path});',
        'const result=await plugin.importSaveBackup({path:saveImportArchiveUri,packageName:saveImportGamePackage});saveImportArchiveUri="";',
        1,
    )
    saves_new = saves_new.replace(
        'catch(e){importStatus.textContent="导入失败：',
        'catch(e){saveImportArchiveUri="";importStatus.textContent="导入失败：',
        1,
    )
    saves_new = saves_new.replace(
        'delArchive.onclick=async()=>{if(!confirm',
        'delArchive.onclick=async()=>{if(saveTransferBusy()){importStatus.textContent=saveTransferBusyMessage();return}if(!confirm',
        1,
    )
    saves_new = saves_new.replace(
        'importArchiveBtn.onclick=async()=>{importArchiveBtn.disabled=true;',
        'importArchiveBtn.onclick=async()=>{if(saveTransferBusy()){importStatus.textContent=saveTransferBusyMessage();return}importArchiveBtn.disabled=true;',
        1,
    )
    saves_new = saves_new.replace(
        'const updateImportState=()=>{const pkg=saveImportGamePackage;importGameBtn.textContent=',

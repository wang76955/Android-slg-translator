...
function triggerReactButton(button){manualIdle=false;const target=button===startButton?(findButton("开始翻译")||button):button;const isStart=button===startButton||button?.textContent?.includes("开始翻译")||target?.textContent?.includes("开始翻译");const disabled=(startButton?.disabled&&target===startButton)||(isStart&&target?.disabled);if(disabled){const snap=readTaskSnapshot();setWorkshopState("ready",{...snap,apiRequired:true});return}target?.dispatchEvent(new MouseEvent("click",{bubbles:true,cancelable:true,view:window}))}

...
WORKSHOP_RUNTIME = r"""
;(()=>{const ID="workshop-runtime";let shell=null,sourceButton=null,startButton=null,observer=null,debounceTimer=0,lastSnapshot="",manualIdle=false,retrying=false;
...
function setWorkshopState(state,payload={}){if(!shell)return;shell.dataset.workshopState=state;shell.setAttribute("data-workshop-state",state);shell.dataset.workshopTask=state==="idle"?"idle":"active";shell.setAttribute("data-workshop-task",shell.dataset.workshopTask);shell.classList.remove("workshop-state-idle","workshop-state-scanning","workshop-state-ready","workshop-state-failed");shell.classList.add(`workshop-state-${state}`);shell.replaceChildren(renderTopbar(state),renderStateBody(state,payload))}

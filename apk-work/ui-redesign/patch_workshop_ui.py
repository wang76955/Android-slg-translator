...
function triggerReactButton(button){manualIdle=false;const target=button===startButton?(findButton("开始翻译")||button):button;const disabled=startButton?.disabled&&target===startButton||target?.disabled&&target===startButton;if(disabled){const snap=readTaskSnapshot();setWorkshopState("ready",{...snap,apiRequired:true});return}target?.dispatchEvent(new MouseEvent("click",{bubbles:true,cancelable:true,view:window}))}

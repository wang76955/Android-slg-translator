...
window.__slgHandleAndroidBack=()=>{if(installedDialog&&!installedDialog.hidden){modalHistoryArmed=false;closeInstalledApps(true);return true}if(sourceDialog&&!sourceDialog.hidden){modalHistoryArmed=false;closeSourceChooser(true);return true}if(settingsOpen){closeSettings();return true}return false};

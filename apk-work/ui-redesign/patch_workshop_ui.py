...
function accumulate(records){const all=root.__slgRenpyCompatibilityRecords||[],unique=new Set,contexts=new Map;for(const record of records||[]){const old=text(record?.text);if(!old)continue;all.push(record);unique.add(old);let set=contexts.get(old)||new Set;set.add([text(record?.speaker),text(record?.identifier),text(record?.kind),text(record?.sourcePath)].join(`\u0000`));contexts.set(old,set)}let collision=0;for(const set of contexts.values())if(set.size>1)collision++;root.__slgRenpyCompatibilityRecords=all;const current=root.__slgRenpyCompatibilityReport;if(current){root.__slgRenpyCompatibilityReport=Object.assign({},current,{uniqueTextCount:unique.size,occurrenceCount:all.length,collisionCount:collision});root.__slgRenpyCompatibilityReportJson=sanitizedJson(root.__slgRenpyCompatibilityReport);render(root.__slgRenpyCompatibilityReport)}}

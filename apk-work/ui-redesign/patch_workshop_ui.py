...
WORKSHOP_RUNTIME = r"""
;(()=>{const ID="workshop-runtime";let shell=null,runtimeRoot=null,settingsShell=null,settingsOpen=false,sourceButton=null,startButton=null,reactApiInput=null,observer=null,debounceTimer=0,lastSnapshot="",manualIdle=false,retrying=false;
...
function setReactInputValue(input,value){if(!input)return;const setter=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,"value")?.set;if(setter)setter.call(input,value);else input.value=value;input.dispatchEvent(new Event("input",{bubbles:true}));input.dispatchEvent(new Event("change",{bubbles:true}))}
function findReactApiInput(){return[...document.querySelectorAll("#root input")].find(el=>el.type==="password"||/api.?key/i.test(el.placeholder||el.getAttribute("aria-label")||""))}
function closeSettings(){settingsOpen=false;if(settingsShell)settingsShell.hidden=true;if(shell)shell.hidden=false}
function openSettings(){settingsOpen=true;manualIdle=true;reactApiInput=reactApiInput||findReactApiInput();if(!settingsShell){settingsShell=document.createElement("section");settingsShell.className="workshop-settings-shell";const top=textNode("header","workshop-task-topbar");const back=textNode("button","workshop-task-back","‹");back.type="button";back.setAttribute("aria-label","返回任务");back.onclick=closeSettings;top.append(back,textNode("h1","","我的设置"));const card=textNode("section","workshop-settings-card");const label=textNode("label","","API Key");const input=document.createElement("input");input.className="workshop-settings-input";input.type="password";input.autocomplete="off";input.value=reactApiInput?.value||"";const save=textNode("button","workshop-settings-save","保存");save.type="button";const helper=textNode("p","workshop-settings-helper","API Key 仅保存在本机。");save.onclick=()=>{reactApiInput=reactApiInput||findReactApiInput();setReactInputValue(reactApiInput,input.value);helper.textContent="已保存 API Key";};card.append(label,input,save,helper);settingsShell.append(top,card);runtimeRoot?.append(settingsShell)}settingsShell.hidden=false;if(shell)shell.hidden=true}
...
function triggerReactButton(button){manualIdle=false;if(startButton?.disabled&&button===startButton){const snap=readTaskSnapshot();setWorkshopState("ready",{...snap,apiRequired:true});return}button?.dispatchEvent(new MouseEvent("click",{bubbles:true,cancelable:true,view:window}))}
...
function readTaskSnapshot(){const text=sourceText();const selected=text.match(/已选择\s*[:：]?\s*([^\n]{1,180}?)(?=发现|正在|处理|$)/);const fileName=selected?.[1]?.trim()||"";const found=text.match(/发现\s*(\d+)\s*个可翻译文件/);const count=found?.[1]||"";const failed=[...document.querySelectorAll("#root *")].find(el=>!el.closest(".workshop-task-shell")&&el.textContent?.includes("写入补丁 APK 失败"));if(failed&&/ENOSPC|No space left/i.test(failed.textContent||""))return{state:"failed",reason:"space",raw:failed.textContent};if(/正在处理脚本|翻译中|开始处理/.test(text))return{state:"scanning",fileName,count};if(count!=="")return{state:"ready",fileName,count};if(fileName||/正在扫描|正在检查文件|检查文件/.test(text))return{state:"scanning",fileName};return{state:"idle"}}
function detailToggle(raw){const wrap=textNode("div","workshop-detail-wrap");const toggle=textNode("button","workshop-detail-toggle","处理详情");toggle.type="button";toggle.setAttribute("aria-expanded","false");const body=textNode("div","workshop-detail-body",raw||"暂无更多信息");body.dataset.open="false";toggle.onclick=()=>{const open=body.dataset.open!=="true";body.dataset.open=String(open);toggle.setAttribute("aria-expanded",String(open));toggle.textContent=open?"收起详情":"处理详情"};wrap.append(toggle,body);return wrap}
function renderTopbar(state){const bar=textNode("header","workshop-task-topbar");const back=textNode("button","workshop-task-back","‹");back.type="button";back.setAttribute("aria-label","返回");back.onclick=()=>setWorkshopState("idle",{fromBack:true});bar.append(back,textNode("h1","","APK 翻译"),textNode("span","workshop-topbar-state",state==="idle"?"":"处理中"));return bar}
...
function setWorkshopState(state,payload={}){if(!shell)return;shell.dataset.workshopState=state;shell.setAttribute("data-workshop-state",state);shell.dataset.workshopTask=state==="idle"?"idle":"active";const task=shell.dataset.workshopTask;shell.setAttribute("data-workshop-task",shell.dataset.workshopTask);runtimeRoot?.setAttribute("data-workshop-state",state);runtimeRoot?.setAttribute("data-workshop-task",task);shell.classList.remove("workshop-state-idle","workshop-state-scanning","workshop-state-ready","workshop-state-failed");shell.classList.add(`workshop-state-${state}`);runtimeRoot?.classList.remove("workshop-state-idle","workshop-state-scanning","workshop-state-ready","workshop-state-failed");runtimeRoot?.classList.add(`workshop-state-${state}`);shell.replaceChildren(renderTopbar(state),renderStateBody(state,payload))}
function snapshotKey(s){return[s.state,s.fileName||"",s.count||"",s.reason||"",s.raw||""].join("|")}

...
function closeSourceChooser(){if(sourceDialog)sourceDialog.hidden=true;previousSourceFocus?.focus()}
function openSourceChooser(){previousSourceFocus=document.activeElement;if(!sourceDialog){sourceDialog=textNode("section","workshop-source-dialog");sourceDialog.hidden=true;sourceDialog.setAttribute("role","dialog");sourceDialog.setAttribute("aria-modal","true");sourceDialog.setAttribute("aria-labelledby","workshop-source-title");const panel=textNode("div","workshop-modal-panel");const title=textNode("h2","workshop-modal-title","选择来源");title.id="workshop-source-title";const copy=textNode("p","workshop-modal-copy","可以从手机里的应用开始，也可以选择一个 APK 文件。");const installed=actionButton("从已安装应用选择",()=>{closeSourceChooser();openInstalledApps()});installed.className="workshop-source-option";const file=actionButton("从文件选择 APK",()=>{closeSourceChooser();triggerReactButton(sourceButton)});file.className="workshop-source-option";const cancel=actionButton("取消",closeSourceChooser,true);cancel.className="workshop-modal-action";panel.append(title,copy,installed,file,cancel);sourceDialog.append(panel);sourceDialog.onclick=event=>{if(event.target===sourceDialog)closeSourceChooser()};sourceDialog.onkeydown=event=>{if(event.key==="Escape"){event.preventDefault();closeSourceChooser()}else trapModalFocus(event,sourceDialog)};runtimeRoot?.append(sourceDialog)}sourceDialog.hidden=false;window.setTimeout(()=>focusableIn(sourceDialog)[0]?.focus(),0)}
...
function renderInstalledApps(){if(!installedDialog)return;const list=installedDialog.querySelector(".workshop-installed-content"),search=installedDialog.querySelector(".workshop-installed-search");if(!list)return;list.replaceChildren();if(installedLoading){list.append(textNode("p","workshop-installed-status","正在读取应用列表"));return}if(installedError){const message=textNode("p","workshop-installed-status workshop-installed-error",`应用列表读取失败：${installedError}`);const retry=actionButton("重新加载",loadInstalledApps);retry.className="workshop-source-option";const file=actionButton("从文件选择 APK",()=>{closeInstalledApps();triggerReactButton(sourceButton)},true);file.className="workshop-modal-action";list.append(message,retry,file);return}const visible=filterInstalledApps(installedApps,search?.value||"");if(!visible.length){list.append(textNode("p","workshop-installed-status","没有找到可选择的应用"));return}const rows=textNode("ul","workshop-app-list");for(const app of visible){const item=textNode("li","");const button=textNode("button","workshop-app-row");button.type="button";button.append(textNode("span","workshop-app-label",app.label||app.packageName),textNode("span","workshop-app-package",app.packageName));button.onclick=()=>chooseInstalledApp(app);item.append(button);rows.append(item)}list.append(rows)}
async function loadInstalledApps(){installedLoading=true;installedError="";renderInstalledApps();try{const result=await window.Capacitor.Plugins.FileManager.listInstalledApps();installedApps=Array.isArray(result?.apps)?result.apps:[]}catch(error){installedError=error?.message||String(error)}finally{installedLoading=false;renderInstalledApps()}}
async function openInstalledApps(){previousSourceFocus=previousSourceFocus||document.activeElement;if(!installedDialog){installedDialog=textNode("section","workshop-installed-dialog");installedDialog.hidden=true;installedDialog.setAttribute("role","dialog");installedDialog.setAttribute("aria-modal","true");installedDialog.setAttribute("aria-labelledby","workshop-installed-title");const panel=textNode("div","workshop-modal-panel");const title=textNode("h2","workshop-modal-title","从已安装应用选择");title.id="workshop-installed-title";const search=document.createElement("input");search.className="workshop-installed-search";search.type="search";search.placeholder="搜索应用名称或包名";search.setAttribute("aria-label","搜索应用名称或包名");search.oninput=renderInstalledApps;const content=textNode("div","workshop-installed-content");const cancel=actionButton("取消",closeInstalledApps,true);cancel.className="workshop-modal-action";panel.append(title,search,content,cancel);installedDialog.append(panel);installedDialog.onclick=event=>{if(event.target===installedDialog)closeInstalledApps()};installedDialog.onkeydown=event=>{if(event.key==="Escape"){event.preventDefault();closeInstalledApps()}else trapModalFocus(event,installedDialog)};runtimeRoot?.append(installedDialog)}installedDialog.hidden=false;installedDialog.querySelector(".workshop-installed-search").value="";window.setTimeout(()=>installedDialog.querySelector(".workshop-installed-search")?.focus(),0);await loadInstalledApps()}
async function chooseInstalledApp(app){try{const selection=await window.Capacitor.Plugins.FileManager.selectInstalledApp({packageName:app.packageName});closeInstalledApps();await window.__slgLoadSelectedApk(selection)}catch(error){installedError=error?.message||String(error);if(installedDialog)installedDialog.hidden=false;renderInstalledApps()}}
function closeInstalledApps(){if(installedDialog)installedDialog.hidden=true;previousSourceFocus?.focus()}
...
document.addEventListener("click",event=>{if(event.target.closest?.(".workshop-task-back"))manualIdle=true},{capture:true});observer=new MutationObserver(schedule);observer.observe(document.querySelector("#root")||document.documentElement,{childList:true,subtree:true,characterData:true});window.addEventListener("popstate",()=>{if(installedDialog&&!installedDialog.hidden){closeInstalledApps();return}if(sourceDialog&&!sourceDialog.hidden){closeSourceChooser();return}if(settingsOpen){closeSettings();return}if(shell?.dataset.workshopTask==="active"){manualIdle=true;setWorkshopState("idle",{fromBack:true})}});document.readyState==="loading"?document.addEventListener("DOMContentLoaded",mount):mount()})();
"""
...
def patch_scan_flow(js: str) -> str:
    ...
    new = """loadSelectedApk=window.__slgLoadSelectedApk=async e=>{try{window.__slgSelectionMeta=e,r(e.uri),a(e.name||e.label||e.uri.split(`/`).pop()||`Unknown.apk`),p(e.packageName||``),s([]),fe(null),w([]),d(!0),O(`正在扫描 APK 中的文本文件...`,`info`),e.splitApk&&O(`该应用使用拆分安装包（${e.splitCount||0} 个拆分包），当前先扫描基础 APK，部分资源可能无法读取。`,`info`);let t=await E.listApkEntries({uri:e.uri});if(e.splitApk&&t.entries.length===0)throw new Error(`该应用使用拆分安装包，基础 APK 中没有可翻译文件。请改用“从文件选择 APK”。`);s(t.entries),O(`APK 检查${t.cacheHit?`（缓存）`:``}用时 ${Math.max(0,Math.round((t.scanDurationMs||0)/100)/10)} 秒`,`info`);try{let n=e.packageName?{packageName:e.packageName}:t.packageName?{packageName:t.packageName}:await Promise.race([E.getApkPackageName({uri:e.uri}),new Promise(e=>setTimeout(()=>e({packageName:``}),3e3))]);n.packageName&&(p(n.packageName),O(`识别到包名: `+n.packageName,`info`))}catch{}let n=Jo(t.entries,c,g),i=Oe(n),o=Object.entries(i).map(([e,t])=>`${ds[e]||e}×${t}`).join(`, `);O(`共发现 ${n.length} 个可翻译文件（${o||`默认仅 Ren'Py`}）`,`success`),d(!1)}catch(t){t.message!==`User cancelled`&&O(`选择文件失败: ${t.message}`,`error`),d(!1);throw t}},xe=async()=>{try{await loadSelectedApk(await E.pickApkFile())}catch{d(!1)}}"""
    if js.count(old) != 1:
...
function setReactInputValue(input,value)""",
    ...
    runtime = runtime.replace(
        's.reason||"",s.raw||""',
        's.reason||"",s.installAvailable?"install":"",s.raw||""',
        1,
    )
    return runtime

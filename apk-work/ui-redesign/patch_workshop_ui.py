...
WORKSHOP_RUNTIME = r"""
;(()=>{const ID="workshop-runtime";const textNode=(tag,cls,text)=>{const el=document.createElement(tag);el.className=cls;el.textContent=text;return el};const findButton=label=>[...document.querySelectorAll("#root button")].find(el=>!el.closest(".workshop-hero")&&el.textContent&&el.textContent.includes(label));const scrollToLabel=label=>{const target=[...document.querySelectorAll("#root h1,#root h2,#root summary")].find(el=>el.textContent&&el.textContent.includes(label));(target||document.querySelector("#root")).scrollIntoView({behavior:"smooth",block:"start"})};const decorate=()=>{const heading=[...document.querySelectorAll("#root h2")].find(el=>el.textContent&&el.textContent.includes("选择游戏 APK"));if(heading&&heading.parentElement)heading.parentElement.classList.add("workshop-picker-source");const startButton=[...document.querySelectorAll("#root button")].find(el=>el.textContent&&el.textContent.includes("开始翻译"));if(startButton)startButton.classList.add("workshop-start-button")};const mount=()=>{const sourceButton=findButton("选择");decorate();const app=document.querySelector("#root>div");if(!app||app.dataset.workshopMounted||!sourceButton)return;app.dataset.workshopMounted="true";app.classList.add(ID);const hero=document.createElement("section");hero.className="workshop-hero";hero.append(textNode("small","","今天翻译什么？"),textNode("h1","","让喜欢的故事，用中文继续。"));sourceButton.classList.add("workshop-source-button");sourceButton.setAttribute("aria-label","选择 APK 文件");const start=textNode("button","workshop-touch","选择 APK 文件");start.type="button";start.onclick=()=>sourceButton.dispatchEvent(new MouseEvent("click",{bubbles:true,cancelable:true,view:window}));hero.append(start);app.prepend(hero);const nav=document.createElement("nav");nav.className="workshop-bottom-nav";nav.setAttribute("aria-label","主导航");[["首页",()=>window.scrollTo({top:0,behavior:"smooth"})],["作品",()=>scrollToLabel("处理详情")],["我的",()=>scrollToLabel("翻译设置")]].forEach(([label,action],index)=>{const button=textNode("button","workshop-touch",label);button.type="button";if(index===0)button.setAttribute("aria-current","page");button.onclick=()=>{[...nav.children].forEach(el=>el.removeAttribute("aria-current"));button.setAttribute("aria-current","page");action()};nav.append(button)});app.append(nav)};new MutationObserver(mount).observe(document.documentElement,{childList:true,subtree:true});document.readyState==="loading"?document.addEventListener("DOMContentLoaded",mount):mount()})();
"""

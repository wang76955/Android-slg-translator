...
For the first inspection of an APK, the helper first opens a seekable `ParcelFileDescriptor` and exposes it to `ZipFile` through the process-local `/proc/self/fd/<fd>` path. This reads the ZIP central directory without copying the APK or reading entry bodies, including for multi-gigabyte local files. Cloud-backed providers that do not expose a seekable descriptor fall back to a bounded temporary copy of compressed bytes. The helper applies the same filename filtering and file-type detection through the existing plugin’s private methods, extracts the package name from `AndroidManifest.xml`, resolves the existing `{entries, totalFiles}` contract, and adds optional timing metadata. Fallback temporary APK bytes are deleted in `finally`.

...
- **证据**：VNDB ~27%（~7,900 款）为最大单桶；新游戏持续发布在 8.4/8.5 上（增长型）。**注意 8.4 与 8.5 是两次不同的序列化变更**：8.4 起 AST 只序列化非默认值；8.5 起 Cython 化 astsupport 全局变量。二者应视为两个子格式，不能共享同一套还原规则假设。
- **现状**：这些游戏落入 `UNKNOWN_EXTRACT_ONLY` —— 能提取、能翻译、能导出，但**不能写回 APK**。
- **方案**：
  1. 建立 8.4 与 8.5 两个子格式各自的真实样本矩阵（优先 8.5.3 最新稳定版），记录 container / slot / pickle protocol / 对象图 / version-key / 启动结果。
  2. **先封闭技术验证**：能否用 fake renpy 包分别重建 8.4 与 8.5 的 AST pickle。8.4 的"只序列化非默认值"是最大变数——需确认默认值还原规则是否可在不依赖原版源码的情况下复现；8.5 需确认 Cython 化后对象图是否仍与 8.4 兼容。
  3. 扩展 `RpycCompatibility` 方言识别，使 8.4/8.5 各自从 `UNKNOWN_EXTRACT_ONLY` 进入可生成候选，再由 writer 承载（可先实现 8.4，8.5 待验证通过）。
  4. 产物重读 → 安装 → 启动 → 中文生效，逐级升级为 `VERIFIED`。
- **验收**：8.4 与 8.5 **各** ≥1 个真实样本端到端通过；未覆盖子格式保持 `TRANSLATABLE_NO_PATCH`，不扩大通配判断。

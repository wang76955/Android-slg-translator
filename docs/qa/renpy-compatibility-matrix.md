...
> 真机复验更新（2026-08-08）：以上结论中的“没有真实样本”是前一阶段快照；本次用户已明确提供合法、完整、可测试的手机本地 APK `异世界天堂0.6翻.apk`。现已确认 `com.yishijietiantang.com` / versionName `0.6` / `versionCode 1755587835`，完成未修改游戏的安装、Ren'Py 启动、主菜单和新游戏基线，并用本地重建 v1.0.7 扫描出 `104` 个 RPYC 文件。翻译、补丁安装、译文语言、字体、old save 和 rollback 仍未完成；真实 `content://` URI 的菜单注入当前为 `FAIL`，详见 T22-04..T22-06。
...
| C16-03 | activation: standard/custom/none menu, selectable vs always-on | `SAFE`/`WARNING` as preflight decides; `selectable` and `always-on` remain separate | generated UI, language injection and C13 nullable-path fixtures; real scan found 104 RPYC files; content-URI menu injection failed | fixture counts only; real `not-run` | real `missing/rejected=not-run` | fixture gate; real `not-run` | tool artifact build PASS; unmodified game startup PASS; translated activation/language/old save/rollback `NOT-RUN` | FAIL for the observed content-URI injection path; controlled contract PASS; T12/T13/T15/T22-06 evidence |
...
| C16-05 | APK shape: `single APK` and `base + split` | inherited source level/activation; split uses one session | `InstalledApkSet`, `InstalledAppSource`, helper DEX and rebuilt workshop APK; real target package `com.yishijietiantang.com` installed as a single APK | fixture counts; real scan has `104` RPYC entries but no coverage report | real coverage/rejected `not-run` | fixture result; real `not-run` | local build/signature/asset gates PASS; SHA-256 `AF5D55B12AD0089C50539B99D9D370B4D197B90CCDD9CECAD83729C84C974295`; v2/v3 true; unmodified real-game install/startup PASS; translated install/old save/rollback `NOT-RUN` | PASS for local artifact and unmodified single-APK baseline; translated device acceptance NOT-RUN; T14/T15/T20/T22-04 evidence |
| C16-06 | final release gate: automated, artifact, real sample and device definition of done | all declared support/activation modes must have evidence; `EXTRACT_ONLY`/`UNSUPPORTED` remain blocked | current evidence index; real sample identity, baseline install/start and scan are recorded; translation/patch/language/font/save/rollback are incomplete | automated fixture counts recorded; real `missing/rejected=not-run` | mandatory real rows are not complete; C16-03 has an observed FAIL | real font final set `not-run` | Task 23 audit and tool artifact gates PASS; full release decision remains `NOT-RUN`; no `批准发布` | NOT-RUN / release blocked by T21-02, T22-02 and T22-06 |
...
|---|---|---|---|---|
| T21-01 | 本地 APK 候选的包名、版本和 Ren'Py 资源入口盘点 | 用户确认手机本地 `异世界天堂0.6翻.apk` 合法、完整且可测试；包名 `com.yishijietiantang.com`，versionName `0.6`，versionCode `1755587835`，入口 `org.renpy.android.PythonSDLActivity`；运行时显示 Ren'Py loading/menu | PASS | 合法性和完整性来自用户明确确认；包身份、入口和运行时行为已独立观察；源 APK 留在手机，不写入仓库 |
| T21-02 | 真实样本 coverage、字体、编译、安装、启动、语言、old save、rollback | 已完成真实 APK 安装/启动基线和 v1.0.7 的 `104` 个 RPYC 扫描；真实 coverage/missing/rejected/font/compiler/translated patch/language/old save/rollback 未完成 | NOT-RUN | 扫描数量不能替代 coverage 结果；翻译因无可用 API Key 未执行，不能宣称 `missing == 0` |
| T22-01 | 已连接设备只读 inventory | `PEMM20`; Android `13`/SDK `33`; ABI `arm64-v8a,armeabi-v7a,armeabi`; `com.slgtranslator.app` 已降级为 versionCode `7`/versionName `1.0.7` 且 dataDir 保持；`com.yishijietiantang.com` versionName `0.6` 已安装 | PASS | 设备、工具版本、数据保留和真实目标身份已验证；不代表翻译目标 APK、语言、字体、old save 或 rollback 已通过 |
| T22-02 | 真机完整工作流 | 完整链路未完成；未修改游戏安装/启动和 v1.0.7 导入/扫描分别见 T22-04/T22-05；翻译、补丁安装、语言、字体、old save、rollback、split session 未执行 | NOT-RUN | 基线子集不能替代真实翻译验收；菜单注入的内容 URI 失败见 T22-06 |
...
| T22-04 | 真实 Ren'Py 游戏单 APK 安装、启动和新游戏基线 | 手机本地 APK 为 `3,504,837,594` bytes；直接从 `/sdcard` 安装受 Android 13 FUSE 权限限制，复制到 `/data/local/tmp/renpy-0.6-device-test.apk` 后 `pm install -r -t` 返回 `Success`；`com.yishijietiantang.com` 的 Ren'Py loading、主菜单 `Ver. 0.6`、新游戏和两段英文对白均可见；截图见 `apk-work/device-renpy-original-launch-20260808-0648.png`、`device-renpy-original-menu-20260808-0651.png`、`device-renpy-original-newgame-20260808-0654.png`、`device-renpy-original-dialogue2-20260808-0656.png` | PASS | 仅为未修改游戏基线；没有译文 APK、语言切换、中文字体、old save、rollback 或 split session 证据；目标日志未见 fatal/ANR |
| T22-05 | v1.0.7 导入真实 Ren'Py APK 并扫描 RPYC | 通过 DocumentsUI 选择 `content://com.android.providers.downloads.documents/document/msf%3A1000009265` 后，v1.0.7 显示 `104 个脚本`、`发现 104 个可翻译文件`，可见 `assets/x-game/x-options.rpyc`、`x-screens.rpyc`、`x-script.rpyc` 等条目并进入 `已就绪`；截图见 `apk-work/device-v7-home-20260808-0630.png`、`apk-work/device-v7-after-real-apk-select-20260808-0636.png`、`apk-work/device-v7-real-scan-scroll-20260808-0638.png`、`apk-work/device-v7-real-settings-bottom-20260808-0640.png` | PASS | 仅证明选择和扫描；无可用 API Key 时 `开始翻译` 未执行，不产生 coverage/missing/patch 结果 |
| T22-06 | `content://` 真实来源的语言菜单注入 | `injectTranslatorMenu` 日志为 `补丁源 APK 不存在: content://com.android.providers.downloads.documents/document/msf%3A1000009265`；`LanguageMenuSupport.fileFrom` 仅处理 `file://`，其余 URI 直接构造 `new File(normalized)`，因此 ZIP 重写器无法读取 DocumentsUI URI；生成 UI 捕获错误后仍显示扫描就绪 | FAIL | 这是可复现的菜单注入失败，不是崩溃也不是 always-on 成功；根因尚未修复，本次不生成或安装补丁 |

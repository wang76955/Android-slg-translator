...
| C16-06 | final release gate: automated, artifact, real sample and device definition of done | all declared support/activation modes must have evidence; `EXTRACT_ONLY`/`UNSUPPORTED` remain blocked | current evidence index and release checklist; real sample source/device prerequisites absent | automated fixture counts recorded; real `not-run` | mandatory real rows remain `NOT-RUN` | real font final set `not-run` | Task 23 audit complete; full release decision remains `NOT-RUN` until Task 21–22 real gates run; no `批准发布` | NOT-RUN / final audit complete, waiting external acceptance |
